...
void setFont(const char* family, int size) {
    ...
    if (fonts.primary) XftFontClose(display, fonts.primary);
    for (auto* f : fonts.fallbacks) if (f) XftFontClose(display, f);
...
void drawStringPlain(const std::string &text, int x, int y, 
    ...
    int text_x = x;
    if (align == ALIGN_CENTER) text_x = x - tm.width / 2;
    else if (align == ALIGN_RIGHT) text_x = x - tm.width;
...
void shutdown() {
    ...
    if (back_buffer) XFreePixmap(display, back_buffer);
